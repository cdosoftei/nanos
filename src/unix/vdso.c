...
void init_vdso(process p)
{
    ...
    map(p->vdso_base, paddr, PAGESIZE, PAGE_USER, pages);

    /* map vvar pages */
    paddr = physical_from_virtual((void *)&vvar_page);
    assert(paddr != INVALID_PHYSICAL);
    map(p->vdso_base + PAGESIZE, paddr, PAGESIZE, PAGE_USER, pages);
    
    paddr = pvclock_get_physaddr();
    assert(paddr != INVALID_PHYSICAL);
    map(p->vdso_base + PAGESIZE*2, paddr, PAGESIZE, PAGE_USER, pages);


    rprintf("mapped VDSO to 0x%lx\n", p->vdso_base);
